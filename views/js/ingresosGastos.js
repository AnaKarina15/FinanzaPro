document.addEventListener("DOMContentLoaded", () => {
  ...
  const aplicarFiltros = () => {
    ...
    const movsFiltrados = movimientosGlobales.filter((mov) => {
      const coincideTipo =
        ...
        mov.tipo === filtroTipoSeleccionado;
    ...
    if (tabla) {
      if (movsFiltrados.length === 0) {
        tabla.innerHTML =
          '<tr><td colspan="5" class="empty-table-msg">No hay transacciones para este filtro.</td></tr>';
      } else {
        tabla.innerHTML = "";
        movsFiltrados.forEach((mov) => {
          const esG = mov.tipo === "gasto";
          const fecha = new Date(mov.fecha + "T00:00:00").toLocaleDateString(
            "es-ES",
  ...
  const actualizarFiltrosVisuales = () => {
    document.querySelectorAll(".filter-option").forEach((el) => {
      el.classList.toggle(
        "filter-active",
        el.dataset.filter === filtroTipoSeleccionado,
      );
    });
    document.querySelectorAll(".filter-time-option").forEach((el) => {
      el.classList.toggle(
        "filter-time-active",
        el.dataset.time === filtroTiempoSeleccionado,
      );
    });
    ...
    if (label) {
      label.innerText =
        document.querySelector(
          `.filter-time-option[data-time="${filtroTiempoSeleccionado}"]`,
        )?.innerText || "Este Mes";
    }
  };

  // --- RENDERIZADO DE GRÁFICAS Y LEYENDAS ---
  function renderizarDonaYleyenda(
    canvasId,
    centerId,
    legendId,
    dataset,
    ...
    if (!dataset || dataset.length === 0) {
      ...
      legendContainer.innerHTML =
        '<p class="empty-table-msg">Sin movimientos</p>';
      new Chart(canvas.getContext("2d"), {
        type: "doughnut",
        data: {
          labels: ["Vacio"],
          datasets: [
            { data: [1], backgroundColor: ["#e2e8f0"], borderWidth: 0 },
          ],
        },
        options: {
          responsive: true,
    ...
    new Chart(canvas.getContext("2d"), {
      ...
      data: {
        ...
        datasets: [
          {
            data: dataset.map((c) => parseFloat(c.total)),
  ...
  fetch("../index.php?action=obtenerEstadisticas")
    ...
    .then((texto) => {
      ...
      document.getElementById("total-ingresos-view").innerText =
        formatearMoneda(totalI);
      document.getElementById("total-gastos-view").innerText =
        formatearMoneda(totalG);

      renderizarDonaYleyenda(
        "graficaIngresos",
        "center-ingresos",
        "legend-ingresos-list",
        data.categoriasIngresos,
        totalI,
      ...
      renderizarDonaYleyenda(
        ...
        totalG,
        ["#ef4444", "#f97316", "#eab308", "#cbd5e1"],
        "gasto",
      );

      movimientosGlobales = Array.isArray(data.movimientos)
        ? data.movimientos
        : [];
      aplicarFiltros();
    })
  ...
  document.addEventListener("click", (e) => {
    ...
    const limpiarFiltros = e.target.closest("#clear-filters");
    if (limpiarFiltros) {
      e.preventDefault();
      filtroTipoSeleccionado = "todos";
      filtroTiempoSeleccionado = "esteMes";
      if (dropdownTime) dropdownTime.classList.remove("open");
      aplicarFiltros();
      return;
    }
  ...
  document.addEventListener("click", (e) => {
    ...
    if (btnEdit) {
      ...
      document.querySelector(
        `input[name="tipo_movimiento"][value="${d.tipo}"]`,
      ).checked = true;
      ...
      document.getElementById("monto_visual").value =
        `$ ${new Intl.NumberFormat("es-CO").format(d.monto)}`;
